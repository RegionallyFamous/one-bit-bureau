...
printf '%s\n' 'if [[ $1 == "eval" && $2 == *"hl.dsp.focus"* && ${FAIL_FOCUS:-0} == 1 ]]; then exit 1; fi' >>"$work/bin/hyprctl"
...
grep -Fq 'eval hl.dsp.focus({ window = "address:0xabc123" })' "$HYPR_LOG"
grep -Fq 'eval hl.dsp.window.bring_to_top()' "$HYPR_LOG"
grep -Fq 'eval hl.dsp.cursor.move({ x = 120, y = 340 })' "$HYPR_LOG"
...
echo -n >"$HYPR_LOG"
if NO_WARPS_INT=0 FAIL_FOCUS=1 PATH="$work/bin:$PATH" bash "$helper" '0xfeed' 2>/dev/null; then
