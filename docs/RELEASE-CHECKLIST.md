...
- Install through a real pseudo-terminal with the literal public command `bash <(curl -fsSL https://bureau.regionallyfamous.com/install)`. Confirm the output explains that Omarchy's initial disabled state is temporary, the bootstrap continues without requiring a second command, and the public plugin checkout contains the exact acceptance test under execution.
