...
- Create the interrupted-install state with `omarchy plugin add https://github.com/RegionallyFamous/one-bit-bureau.git --yes`, then install through a real pseudo-terminal with the literal public command `bash <(curl -fsSL https://bureau.regionallyfamous.com/install)`. Confirm the bootstrap recognizes the canonical clean checkout, disables it if necessary, updates it, explains that Omarchy's disabled state is temporary, continues without manual cleanup or a second command, and contains the exact acceptance test under execution.
