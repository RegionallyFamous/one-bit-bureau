...
- Run the secure-I/O regressions and verify install-state, installed-command, dock/desktop state, and GTK transactions hold no-follow parent descriptors; bind opened owner/type/mode/link/identity/hash checks to replacement; use random exclusive temporary files; `fsync` files and directories; and reject exact-limit-plus-one bytes, multibyte overflow, excessive depth/keys/nodes/files/directories, malformed JSON, symlinks, hardlinks, and FIFOs without partial mutation.
